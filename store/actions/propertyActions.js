...
export const createProperty= (id,name,address,region,wilaya,balls,showers,bibs,rooms,roof,referee,owner_id) =>{
    ...
    return async dispatch =>{
        ...
        try{
            const response= await fetch('http://192.168.1.34:3000/property/addProperty',{
                method : "POST",
                headers: {
...
export const setProperties= ()=>{
    ...
    return async dispatch =>{
      ...
      try{
           const response= await fetch('http://192.168.1.34:3000/property');
           if(!response.ok){
            throw new Error('Oups! Une erreur est survenue.');
            }
...
export const createPropertyStadiums = (numStadium5x5,numStadium6x6,numStadium7x7,numStadium8x8,numStadium9x9,numStadium10x10,numStadium11x11,type5x5,type6x6,type7x7,type8x8,type9x9,type10x10,type11x11,property_id) =>{
    ...
    return async dispatch =>{
        ...
        try{
            const response= await fetch(`http://192.168.1.34:3000/property/addStadium/${numStadiumString}`,{
                method : "POST",
                headers: {
                    'Content-Type': 'application/json'
                },
...
export const setStadiums= ()=>{
    ...
    return async dispatch =>{
      ...
      try{
           const response= await fetch('http://192.168.1.34:3000/stadium');
           if(!response.ok){
            throw new Error('Oups! Une erreur est survenue.');
            }
...
export const setPropertyStadiums = ()=>{
    ...
    return async dispatch =>{
        ...
        try{
             const response= await fetch('http://192.168.1.34:3000/propertyStadiums');
             if(!response.ok){
              throw new Error('Oups! Une erreur est survenue.');
              }
...
export const updateProperty= (id,name,addressP,region,wilaya,ownerid) => {
    ...
    return async dispatch => {
         ...
         try{
           const response = await fetch(`http://192.168.1.34:3000/property/updateProperty/${ownerid}`,{
              method:'PATCH',
              headers: {
                'Content-Type': 'application/json'
            },
